    ...
    for batch_idx in it:
        ...
        with nemo.core.typecheck.disable_checks():
            ...
            beams_batch = beam_search_lm.forward(
                log_probs=probs_batch, log_probs_length=None,
            )
        ...
        for beams_idx, beams in enumerate(beams_batch):
            ...
            for candidate_idx, candidate in enumerate(beams):
                if ids_to_text_func is not None:
                    ...
                    pred_text = ids_to_text_func(
                        [ord(c) - TOKEN_OFFSET for c in candidate[1]]
                    )
                else:
...
def main():
    ...
    parser.add_argument(
        ...
        default=None,
        type=str,
        help="The path of the KenLM binary model file",
    )
    parser.add_argument(
        "--input_manifest",
        required=True,
        type=str,
        help="The manifest file of the evaluation set",
    )
    parser.add_argument(
        "--preds_output_folder",
        default=None,
        type=str,
        help="The optional folder where the predictions are stored",
    )
    parser.add_argument(
        "--probs_cache_file",
        default=None,
        type=str,
        help="The cache file for storing the outputs of the model",
    )
    parser.add_argument(
        "--acoustic_batch_size",
        default=32,
        type=int,
        help="The batch size to calculate log probabilities",
    )
    parser.add_argument(
        "--device",
        default="cuda",
        type=str,
        help="The device to load the model onto to calculate log probabilities",
    )
    parser.add_argument(
        "--use_amp",
        action="store_true",
        help="Whether to use AMP if available to calculate log probabilities",
    )
    parser.add_argument(
        "--decoding_mode",
    ...
    parser.add_argument(
        ...
        default=256,
        type=int,
        help="The batch size to be used for beam search decoding",
    )
    ...
    with open(args.input_manifest, "r") as manifest_file:
        audio_file_paths = []
        for line in tqdm(
            manifest_file, desc=f"Reading Manifest {args.input_manifest} ...", ncols=120
        ):
    ...
    if args.probs_cache_file and os.path.exists(args.probs_cache_file):
        logging.info(
            f"Found a pickle file of probabilities at '{args.probs_cache_file}'."
        )
        logging.info(
            f"Loading the cached pickle file of probabilities from '{args.probs_cache_file}' ..."
        )
        ...
        with autocast():
            with torch.no_grad():
                all_logits = asr_model.transcribe(
                    audio_file_paths, batch_size=args.acoustic_batch_size, logprobs=True
                )
        all_probs = [kenlm_utils.softmax(logits) for logits in all_logits]
        if args.probs_cache_file:
            logging.info(
                f"Writing pickle files of probabilities at '{args.probs_cache_file}'..."
            )
    ...
    if args.decoding_mode == "beamsearch_ngram":
        if not os.path.exists(args.kenlm_model_file):
            raise FileNotFoundError(
                f"Could not find the KenLM model file '{args.kenlm_model_file}'."
            )
